const CONFIG = {
  SOURCE_CALENDAR_NAME: "Viajes con destino",
  TRAVEL_CALENDAR_NAME: "Bloques de viaje",
  DEFAULT_ORIGIN: "Uruguay, Montevideo, Comercio 2073",
  MAPS_API_KEY: "AIzaSyDigeeZpxkZj7x8qhJIPc-WwbAOqzMZtf4",
...
function procesarNuevosEventos() {
  ...
  if (!sourceCalendar) {
    console.error(`❌ No se encontró el calendario: "${CONFIG.SOURCE_CALENDAR_NAME}"`);
    return;
  }
  if (!travelCalendar) {
    console.error(`❌ No se encontró el calendario: "${CONFIG.TRAVEL_CALENDAR_NAME}"`);
    return;
  }
  ...
  console.log(`📅 Encontrados ${eventos.length} eventos en "${CONFIG.SOURCE_CALENDAR_NAME}"`);
...
function procesarEvento(evento, travelCalendar) {
  ...
  const resumenLineas = lineas.length > 0 ? lineas.join("\n") : "Sin información de líneas"; // ← línea que faltaba
  ...
  const eventoIda = travelCalendar.createEvent(travelEventTitle, salidaIda, llegadaIda, {
  ...
  console.log(`✅ Bloque IDA: ${formatearHora(salidaIda)} → ${formatearHora(llegadaIda)}`);

  if (CONFIG.ADD_RETURN_TRIP) {
    const resultadoVuelta = calcularTiempoViaje(lugar, CONFIG.DEFAULT_ORIGIN, fin);
    const duracionVuelta = resultadoVuelta ? resultadoVuelta.duracionMinutos : duracionMinutos;
    const resumenLineasVuelta = resultadoVuelta && resultadoVuelta.lineas.length > 0  // ← también corregido
    ...
    const eventoVuelta = travelCalendar.createEvent(travelReturnTitle, salidaVuelta, llegadaVuelta, {
    ...
    console.log(`✅ Bloque VUELTA: ${formatearHora(salidaVuelta)} → ${formatearHora(llegadaVuelta)}`);
  }
}
...
function calcularTiempoViaje(origen, destino, horaLlegada) {
  ...
  const url = `https://maps.googleapis.com/maps/api/directions/json` +
    ...
    (CONFIG.TRANSIT_PREFERENCE ? `&transit_routing_preference=${CONFIG.TRANSIT_PREFERENCE}` : "") +
...
function crearTrigger() {
  ...
  ScriptApp.newTrigger("procesarNuevosEventos")
    ...
    .everyHours(12)
    .create();
  console.log("✅ Trigger creado: se ejecutará cada hora");
}